...
export function GooglePlayBadge({ className = "", locale = "en" }: { className?: string; locale?: LandingLocale }) {
  return (
    <a
      href={playStoreUrl}
      aria-label={locale === "ar" ? "حمّل وقت الصلاة من Google Play" : locale === "fr" ? "Télécharger Waqto Salat sur Google Play" : locale === "id" ? "Unduh Waqto Salat dari Google Play" : "Get Waqto Salat on Google Play"}
    ...
    >
      <picture>
        ...
        <img
          ...
          alt={locale === "ar" ? "متوفر على Google Play" : locale === "fr" ? "Disponible sur Google Play" : locale === "id" ? "Tersedia di Google Play" : "Get it on Google Play"}
          width="260"
          height="101"
