...
export function MockupSlider({ locale = "en" }: { locale?: LandingLocale }) {
  ...
  const hasMultipleScreenshots = localizedScreenshots.length > 1;
  const current = localizedScreenshots[currentIndex];

  return (
    <button
      type="button"
      aria-label={locale === "ar" ? "لقطة شاشة لتطبيق وقت الصلاة" : locale === "fr" ? "Afficher la capture d'écran Waqto Salat suivante" : locale === "id" ? "Tampilkan tangkapan layar Waqto Salat berikutnya" : "Show next Waqto Salat screenshot"}
      disabled={!hasMultipleScreenshots}
      className={`relative w-[min(20rem,82vw)] aspect-[941/1672] rounded-[2rem] border-[8px] border-border-light shadow-2xl overflow-hidden bg-black shrink-0 flex flex-col items-center justify-center group focus:outline-none focus-visible:ring-4 focus-visible:ring-islamic-gold/40 ${hasMultipleScreenshots ? "cursor-pointer" : "cursor-default"}`}
      onClick={hasMultipleScreenshots ? () => setCurrentIndex((prev) => (prev + 1) % localizedScreenshots.length) : undefined}
    >
